...
*data of 938 people was downloaded from website by using wget command*
...
**For my project I used data of 238 people, and used file that contains information about genotypes**

```{r}
genotypes<-read.table("/Users/bioinf/Desktop/Final_project/example/HGDP_238_chr22.geno",sep="\t", header=FALSE)
```
**Then 1000 variations were subselected, and dataframe "selectedgenotypes was transposed"**
```{r}
library(dplyr)
...
**For Principal component analyse the prcomp function was used**
```{r}
principal_components <- prcomp(selectedgenotypes, center = TRUE,scale. = TRUE)
plot(principal_components, type = "l")
```
**New dataframe "pcadf" was created**
...
```{r}
for_shiny <- plot_ly(pcadf, x =~PC1, y = ~PC2, z = ~PC3,  color = samples$V1.y, colors = "Set1" ) %>%
 add_markers() %>%
